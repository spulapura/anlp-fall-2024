...
rather than a linear-time search for all trigrams keys containing the context.
"""
...
# Returns dictionary of all possible trigrams
# with counts all initialized to 0
...
"""
Given a list of potential alphas, calculate the alpha value that minimizes
the perplexity of the add-alpha-smoothing model built from the given trigram counts,
...
This function selects the language whose input model minimizes the perplexity on 
the input document. 
"""
...
def generate_from_LM(distribution, N):
    context = "##"
    full_doc = "" #In office hours, we were advised not to include the # in our character count
    ...
    while(i < N):
        ...
        probs /= probs.sum() #to resolve slight rounding issues, we normalize
...
def generate_from_LM(distribution, N):
    context = "##"
    full_doc = "" # Per advice in office hours, we do not include # in output
