...
def run_all(preprocessed_lines, devset_preprocessed_lines):
    #Get trigram counts and context counts for training set
    trigram_counts = build_trigram_counts(preprocessed_lines, init_trigram_counts)
    context_counts = build_context_counts(trigram_counts)
    ...
    min_alpha = test_alphas(alphas, trigram_counts, context_counts, vocab_size, devset_preprocessed_lines)

    #Compute optimized add-alpha distribution
    opt_add_alpha_dist = add_alpha_smoothing(trigram_counts, context_counts, vocab_size, min_alpha)
    opt_add_alpha_conditional_probs = build_conditional_prob_distribution(opt_add_alpha_dist)

    #Compare perplexity on 
    print("opt alpha:" + str(min_alpha))
    print(compute_perplexity(opt_add_alpha_conditional_probs, devset_preprocessed_lines))


print("English set")
run_all(en_preprocessed_lines, en_devset_preprocessed_lines)
run_all(en_preprocessed_lines, es_devset_preprocessed_lines)
run_all(en_preprocessed_lines, de_devset_preprocessed_lines)

print("Spanish set")
run_all(es_preprocessed_lines, en_devset_preprocessed_lines)
run_all(es_preprocessed_lines, es_devset_preprocessed_lines)
run_all(es_preprocessed_lines, de_devset_preprocessed_lines)

print("German set")
run_all(de_preprocessed_lines, en_devset_preprocessed_lines)
run_all(de_preprocessed_lines, es_devset_preprocessed_lines)
run_all(de_preprocessed_lines, de_devset_preprocessed_lines)
...
** When doing generation, remove the # marks. Newline characters count towards the 300 total, # does not
** Don't need to add anything to the end of the sentence, 300 characters is sufficient
"""